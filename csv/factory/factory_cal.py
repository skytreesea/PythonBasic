...
# 전체 상위 5개 출력
df2 = df.sort_values(by = '전용면적당거래금액', ascending = False).loc[:5,('시군구','건축물주용도','용도지역','전용/연면적(㎡)', '대지면적(㎡)', '거래금액(만원)', '전용면적당거래금액','용적률')]
# 상위 5개 제외
df3 = df.sort_values(by = '전용면적당거래금액', ascending = False).loc[5:,('시군구','건축물주용도','용도지역','전용/연면적(㎡)', '대지면적(㎡)', '거래금액(만원)', '전용면적당거래금액','용적률')]
