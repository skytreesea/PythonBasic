...
df2['용적률'].describe().to_csv(base_url + r'\용적률test_v1.csv', encoding = 'utf=8-sig')

result = df.pivot_table(values= '대지면적당거래금액', index='도로조건', columns='건축물주용도', aggfunc='mean')
print(result)
result.to_csv(base_url + r'\도로조건_건축물주용도_대지면적당거래금액_v1.csv', encoding = 'utf=8-sig')
result = df.pivot_table(values= '대지면적당거래금액', index='용도지역', columns='건축물주용도', aggfunc='mean')
print(result)
result.to_csv(base_url + r'\용도지역_건축물주용도_대지면적당거래금액_v1.csv', encoding = 'utf=8-sig')
